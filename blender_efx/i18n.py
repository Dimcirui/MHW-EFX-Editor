...
STRINGS = {
    ...
    "material.no_paths":     {"EN": "(no texture paths; parameter-only sets)", "ZH": "（无贴图路径；纯参数 set）"},
    "material.set_count":    {"EN": "{p} texture slot(s), {n} parameter set(s)", "ZH": "{p} 个贴图槽，{n} 个参数 set"},
    "material.parse_fail":   {"EN": "Cannot parse MATERIAL structure (kept opaque)", "ZH": "无法解析 MATERIAL 结构（保持 opaque）"},
