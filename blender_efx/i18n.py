...
STRINGS = {
    ...
    # ── TIML 浏览 / FreeKinetics 快捷编辑（timl_preview） ──────────────────────
    "timlp.no_fk":            {"EN": "Install FreeKinetics to unlock TIML viewport browse & quick edit",
                              "ZH": "安装 FreeKinetics 后解锁 TIML 视口浏览与快捷编辑"},
    "timlp.timeline_hint":   {"EN": "Imports TIML as a mesh Action via FreeKinetics; scrub the timeline to view",
                              "ZH": "借 FreeKinetics 把 TIML 导入成网格动作，拖时间轴查看"},
    "timlp.bind_hint":       {"EN": "Drives the mesh bound on the body's MESH block (transform only for now)",
                              "ZH": "驱动 body 的 MESH 块上绑定的网格（当前仅 transform 通道）"},
    "timlp.scope_label":     {"EN": "Scope (default: active body only)", "ZH": "作用域（默认仅当前特效体）"},
    "timlp.all_bodies":      {"EN": "All bodies in this EFX", "ZH": "本 EFX 所有特效体"},
    "timlp.all_efx":         {"EN": "All EFX in scene", "ZH": "所有 EFX"},
    "timlp.enter":           {"EN": "Browse (read-only)", "ZH": "浏览（只读）"},
    "timlp.enter_edit":      {"EN": "Quick Edit (writes back on apply)", "ZH": "快捷编辑（应用时回写）"},
    "timlp.exit":            {"EN": "Exit (discard)", "ZH": "退出（丢弃）"},
    "timlp.apply_exit":      {"EN": "Apply & Exit (write back)", "ZH": "应用并退出（回写）"},
    "timlp.previewing":      {"EN": "Browsing {0} bound mesh(es)", "ZH": "正在浏览 {0} 个已绑定网格"},
    "timlp.entered":         {"EN": "TIML browse started ({0} mesh)", "ZH": "已进入 TIML 浏览（{0} 个网格）"},
    "timlp.skipped_no_transform": {"EN": "({0} skipped: no transform channel)", "ZH": "（{0} 个无 transform 通道已跳过）"},
    "timlp.exited":          {"EN": "TIML browse ended, meshes restored", "ZH": "已退出 TIML 浏览，网格已还原"},
    "timlp.no_content":      {"EN": "Nothing to browse (need a body with TIML and a bound mesh)",
                              "ZH": "没有可浏览的内容（需要含 TIML 且已绑定网格的 body）"},
    "timlp.no_root":         {"EN": "EFX root not found (select an EFX body/block)",
                              "ZH": "找不到 EFX 根集合（请选中某个 EFX 的 body/块）"},
    "timlp.import_failed":   {"EN": "FreeKinetics import failed: {0}", "ZH": "FreeKinetics 导入失败：{0}"},
    "timlp.writeback_ok":    {"EN": "Wrote back {0} TIML segment(s)", "ZH": "已回写 {0} 个 TIML 段"},
    "timlp.writeback_partial": {"EN": "Wrote back {0}, {1} failed (open the FreeKinetics node editor, or use Replace TIML)",
                              "ZH": "回写 {0} 个，{1} 个失败（请打开 FreeKinetics 节点编辑器，或用「替换 TIML」手动回填）"},
