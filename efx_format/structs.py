...
_UVSEQUENCE_FIXED_SCHEMA = [
    ...
    #   flipCode（bit2-5）：全语料实测确认 11 种取值：0=正常/1=左右翻转/2=随机(正常,左右)/
    #     4=上下翻转/5=左右+上下都翻转/6=上下翻转+随机(正常,左右)/7=同 6/8=随机(正常,上下)/
    #     9=左右翻转+随机(正常,上下)/10=随机(4 种左右×上下组合)/12=正常（完全不翻转）/
    #     13=翻转（固定，非随机）。
    #     ⚠ 曾假设 flipCode 是 4 个独立标志位（+1 强制左右/+2 随机左右覆盖+1/+4 强制上下/
    #     +8 随机上下覆盖+4），6/7 视觉重复一度验证了这个"覆盖"假说——但 12(=+4+8)、
    #     13(=+1+4+8) 打破了它：按覆盖假说 12 该等于 8（随机上下）、13 该等于 9（左右翻转+
    #     随机上下），实测却都是"确定性"结果（12=完全正常，13=固定翻转，都不带随机）。
    #     说明"覆盖"规律不是全局一致的位运算，flipCode 目前只能当经验查表用，不能套公式。
    #     未测：3/11/14/15。3 最关键——测它能看出"左右轴"的 (force+randomize 同时置位)
    #     状态是不是也表现成"确定性/取消"（跟 12 类似），还是表现成"随机"（跟 6/7 类似），
    #     直接判断 6/7 的"覆盖"解读是否也该推翻。
