# -*- coding: utf-8 -*-
"""位置历史的裁剪与按弧长重采样，供条带类渲染体使用。

`Particle.trail` 存逐帧位置，条带需要的是按弧长均分的点：粒子快时一帧走很远、
慢时几乎不动，直接拿帧点当顶点会让分段疏密随速度乱跳。

维护约束：
- 本模块是模拟里调用最密的一段（每条带每帧一趟，上千条带同时活着是常态），
  故全程在平铺 float 上算，只在返回时构造 `Vec3`。不要为可读性把中间结果改回
  `Vec3` 运算。
- `clip_resample()` 是条带用的合并入口，裁剪、算弧长、重采样一趟走完，避免把弧长
  算两遍。
- numpy 是可选加速而非依赖：`clip_resample_many` 有 numpy 走整批数组、没有则逐条
  退回标量路，两条路结果一致（`tools/trail_batch_diff.py` 对拍）。模块本身必须能在
  没有 numpy 的环境里 import。
"""
